...
from hospital.models import Notification, CareTaker
from django.utils import timezone
from django.contrib.auth.decorators import login_required
from django.http import HttpResponseBadRequest
from django.core.mail import send_mail
from .Ocr import handle_conversation
...
def add_notification(request):
    ...
    if request.method == 'POST':
        ...
        notify_obj = Notification.objects.create(
            user=request.user,
        ...
        handle_conversation(request, f"The user (user id: { request.user.id } and user name: { request.user }) need to take a tablet the quantity of { quantity } at { notify_time } the tablet name is { tablet_name } and the total quantity at the data update is { total_tablet_quantity } the updated time is { notify_obj.last_updated_time }... just remember that when i will ask about tablet just give me the details of specified data's detail.")
        print("addded.......!")
